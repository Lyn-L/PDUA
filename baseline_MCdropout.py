...
'''
# python evaluate.py --network resnet50 --adv_im perturbations/resnet152_texture_no_data.npy --img_list ~/dataset/image_name.txt --gt_labels ~/dataset/gt_labels.txt
...
def train(adv_net, net, in_im, ad_im, opt_layers, net_name, train_type, batch_size, K_mc, img_list_file=None):
        ...
        for i in range(max_iter):
            ...
            if not stop_check and ((check_dif > 200 and rescaled == True) or check_dif == 400):
                ...
                if current_rate > fool_rate:
                    ...
                    im = sess.run(ad_im)
                    name = 'perturbations/'+net_name+'_md_'+train_type+'.npy'
                    np.save(name, im)
                else:
                    stopping += 1
                if stopping == 10:
...
def main():
    ...
    parser.add_argument('--k_mc', default=20,
                        help='The batch size to use for training and testing')
    ...
    train(adv_net, net, inp_im, ad_im, opt_layers, args.network,
          args.prior_type, int(args.batch_size), int(args.k_mc), args.img_list)
